...
**Names lag behind semantics.** `WhatsappMessage.phone_number`, the `phone` variable in `connect_event_handler.process_message`, and the log line `"Found existing connection for Phone Number..."` all handle BSUIDs now. Only `from_phone_number` is guaranteed to be a phone number. Renaming these is a good follow-up; leaving them is a good way to confuse the next person reading the code.

**Conversation continuity across the cutover.** A conversation that started before BSUIDs appeared is keyed by phone number. Once Meta starts sending `from_user_id` for that user, the lookup on the `customerId` GSI misses and a new Amazon Connect contact is created. There is no migration code for this in the sample. If you need continuity, the fix is a second lookup: try the BSUID, then fall back to `from_phone_number` when it's present, and rewrite the stored `customerId` on the hit.

**BSUIDs are regenerated when a user changes their phone number.** Meta reports this on the `messages` field as a system message, with `system.type` set to `user_changed_number` or `user_changed_user_id`, and `system.previous_user_id` carrying the old value. That's your join key for stitching the new identity onto the record you already have. This solution does not handle `type: "system"` messages yet — if identity continuity matters to you, that's the next handler to write.

**Outbound payloads are logged.** `print(kwargs)` in `reaction()` and both `send_whatsapp_*` functions writes the full outbound payload to Amazon CloudWatch Logs, including the recipient identifier. BSUIDs and phone numbers are both customer identifiers; review your log retention and access accordingly before this reaches production.

**Agent-initiated outbound still assumes phone numbers.** The companion [agent-initiated WhatsApp](https://github.com/aws-samples/sample-whatsapp-end-user-messaging-connect-chat/tree/main/agent-initiated-whatsapp) solution addresses customers by phone number only. That's fine for its use case — an agent typing a number into a form — but if you want agents to reach a BSUID-only customer proactively, that path needs the same `get_recipient()` treatment.
