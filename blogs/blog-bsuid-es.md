...
**Nombres.** `WhatsappMessage.phone_number`, la variable `phone` en `connect_event_handler.process_message` y la línea de log `"Found existing connection for Phone Number..."` transportan el valor de identidad, que ahora puede ser un BSUID. Solo `from_phone_number` es siempre un número de teléfono. Renombrarlos cuando pases por ahí mantiene la intención clara para quien lea el código después.

**Continuidad de la conversación durante la transición.** Una conversación que empezó antes de que aparecieran los BSUID está indexada por número, así que cuando Meta empiece a enviar `from_user_id` para ese usuario, la búsqueda en el GSI `customerId` no encuentra nada y se inicia un nuevo contacto en Amazon Connect. Para que esos hilos se mantengan, agrega una búsqueda de respaldo: intenta primero con el BSUID, luego con `from_phone_number` cuando esté presente, y reescribe el `customerId` almacenado cuando haya coincidencia.

**Los BSUID se regeneran cuando un usuario cambia su número.** Meta reporta esto en el campo `messages` como un mensaje de sistema, con `system.type` en `user_changed_number` o `user_changed_user_id` y con `system.previous_user_id` llevando el valor anterior — tu clave para unir la nueva identidad al registro que ya tenías. Una rama para mensajes con `type: "system"` es el lugar natural para manejarlo.

**Los payloads salientes incluyen el identificador del destinatario.** Tanto los BSUID como los números identifican a un cliente, así que si conservas el logging de depuración en los caminos de envío, considera la retención y los permisos de acceso a Amazon CloudWatch Logs en tu revisión previa a producción.

**Contacto proactivo.** La solución complementaria de [WhatsApp iniciado por el agente](https://github.com/aws-samples/sample-whatsapp-end-user-messaging-connect-chat/tree/main/agent-initiated-whatsapp) direcciona a los clientes por número, lo que encaja con su caso de uso de un agente escribiendo un número en un formulario. Para que los agentes puedan contactar a un cliente del que solo tienes BSUID, aplica el mismo helper `get_recipient()` en ese camino.
